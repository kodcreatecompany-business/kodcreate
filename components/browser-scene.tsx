...
function createTexture(image: HTMLImageElement, mobile = false) {
  ...
  if (mobile) {
    ...
    ctx.fillStyle = "#f1f4f7"; ctx.fillRect(0, 0, 1440, 64)
    ctx.fillStyle = "#c1ccd8"
    for (let i = 0; i < 3; i++) { ctx.beginPath(); ctx.arc(30 + i * 27, 32, 7, 0, Math.PI * 2); ctx.fill() }
    ctx.fillStyle = "#ffffff"; ctx.beginPath(); ctx.roundRect(440, 15, 560, 33, 8); ctx.fill()
    ctx.fillStyle = "#73808f"; ctx.textAlign = "center"; ctx.font = "16px Arial"; ctx.fillText("forma — architecture & interiors", 720, 38); ctx.textAlign = "left"
    ...
    ctx.fillStyle = "#283b2a"; ctx.fillRect(60, 778, 288, 68); ctx.fillStyle = "#ffffff"; ctx.font = "20px Arial"; ctx.fillText("Explore our spaces   ↗", 89, 820)
...
function FloatingBrowsers({ onReady, onFailure, mouse }: { onReady: () => void; onFailure: () => void; mouse?: { x: number; y: number } }) {
  ...
  return <group ref={group} rotation={[0.11, -0.3, 0.075]}>
    <RoundedBox args={[5.7, 4.01, 0.16]} radius={0.1} smoothness={4}><meshStandardMaterial color="#b8d0ed" roughness={0.3} metalness={0.15} /></RoundedBox>
    ...
    <group position={[2.05, -1.35, 0.78]} rotation={[0, -0.035, -0.11]}>
      <RoundedBox args={[1.14, 2.27, 0.14]} radius={0.13} smoothness={4}><meshStandardMaterial color="#f9fbff" roughness={0.35} /></RoundedBox>
      <mesh position={[0, 0, 0.081]}><planeGeometry args={[1.02, 2.04]} /><meshBasicMaterial map={textures[1]} toneMapped={false} /></mesh>
      <mesh position={[0, 1.062, 0.085]}><capsuleGeometry args={[0.015, 0.17, 3, 8]} /><meshBasicMaterial color="#a8b2bd" /></mesh>
    </group>
  </group>
}
